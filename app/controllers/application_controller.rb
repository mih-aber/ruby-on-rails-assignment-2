class ApplicationController < ActionController::Base
  ...
  before_action :load_actioncable_broadcasts
  ...
  def load_actioncable_broadcasts
    begin
      @actioncable_broadcasts = Broadcast.joins(:feeds).where(:feeds => { :name => 'actioncable' } ).limit(20).order('created_at DESC').to_a
    rescue => e
      @actioncable_broadcasts = []
      end
  end
